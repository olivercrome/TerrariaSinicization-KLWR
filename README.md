...
└── .github/workflows/               # GitHub Actions 工作流
    ├── build-biaozhun.yml           # 发布版 -> data_biaozhun.unity3d（push 自动 + 回写官方更新）
    ├── build-zititihuan.yml         # 自用版 -> data_zititihuan.unity3d（push 自动 + 回写官方更新）
    └── build-siyong.yml             # 私用版 -> data_siyong.unity3d（仅手动触发）
```

> **字体配置说明**（`font_work/`）
>
> | 配置 | 用途 | 死亡/鼠标文字字符集 | 包名 | 触发方式 |
> |---|---|---|---|---|
> | `config-BIAOZHUN.json` | 发布版 | 7000 基础字库 + 顺次补足 | `data_biaozhun.unity3d` | push 自动 / 手动 |
> | `config-ZITITIHUAN.json` | 自用版 | 7000 基础字库 + 顺次补足到 13000 | `data_zititihuan.unity3d` | push 自动 / 手动 |
> | `config-SIYONG.json` | 私用版 | 7000 基础字库 + **14000 字字库** | `data_siyong.unity3d` | **仅手动** |
>
...
>
> 两个 push 自动的工作流共用 `concurrency` 组串行执行，避免同时回写 `Localization/` 时冲突。
...
1. 进入仓库的 **Actions** 标签页
2. 选择最新的工作流运行（`Build BIAOZHUN` / `Build ZITITIHUAN` / `Build SIYONG`）
3. 在页面底部的 **Artifacts** 区域找到对应产物：
   - `data-biaozhun-unity3d`（发布版）
   - `data-zititihuan-unity3d`（自用版）
   - `data-siyong-unity3d`（私用版，需手动触发）
